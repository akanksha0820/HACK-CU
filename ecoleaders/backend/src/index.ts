...
// Default: run in in-memory mode. Set USE_DB=true to connect to Mongo.
const skipDb = process.env.USE_DB === 'true' ? false : true;
...
if (!skipDb) {
  ...
  mongoose
    ...
    .then(async () => {
      ...
      if (process.env.AUTO_SEED !== 'false') await seedIfEmpty();
    })
    .catch((err) => {
  ...
  console.log('USE_DB not set -> running in in-memory demo mode (no Mongo connection).');
}
...
async function seedIfEmpty() {
  ...
  if (userCount === 0) {
    ...
    await User.insertMany(mockUsers.map((u) => ({ name: u.name, email: u.email, password, role: u.role, completedTrainings: u.completedTrainings || [] })));
